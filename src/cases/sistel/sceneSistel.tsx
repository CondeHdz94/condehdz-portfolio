...
function AcmeLogo({ size = 28, color: colorProp }: { size?: number; color?: string }) {
...
function PptSlide({
  ...
  return (
    <div style={{
      width, height, background: '#FFFFFF', border: '1px solid #C9C2B2',
      boxShadow: shadow ? '0 24px 60px rgba(0,0,0,0.18),0 4px 14px rgba(0,0,0,0.08)' : 'none',
      fontFamily: F_PPT, color: '#262626',
      display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden',
      transform: sx,
    }}>
      <div style={{ background: 'linear-gradient(180deg,#1F4E79 0%,#174063 100%)', color: 'white', padding: '22px 38px', fontSize: 30, fontWeight: 700, letterSpacing: '-0.005em', borderBottom: '4px solid #4472C4' }}>
        {title}
      </div>
...
function PptThumbnail({ width = 220, height = 140 }: { width?: number; height?: number }) {
  return (
    <div style={{ width, height, background: '#fff', border: '1px solid #C9C2B2', boxShadow: '0 6px 16px rgba(0,0,0,0.10)', fontFamily: F_PPT, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
...
function Debris({ x, y, rot = 0, w = 60, h = 8, color = '#4472C4', opacity = 1 }: {
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: y, width: w, height: h, background: color, transform: `rotate(${rot}deg)`, opacity, borderRadius: 1, boxShadow: '0 2px 4px rgba(0,0,0,0.10)' }} />
  )
}
...
function Caption({ children, kicker }: { children: React.ReactNode; kicker?: string }) {
  return (
    <div style={{ fontFamily: F_UI, color: '#161310' }}>
      {kicker && (
        <div style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#8E867A', marginBottom: 6, fontWeight: 600 }}>
          {kicker}
        </div>
      )}
      <div style={{ fontFamily: F_DISPLAY, fontSize: 22, lineHeight: 1.15, fontWeight: 400, letterSpacing: '-0.02em' }}>
        {children}
      </div>
    </div>
  )
...
function StorylineEditor({ width = 1520, height = 900, build = 1, children }: {
  ...
  return (
    <div style={{ width, height, background: '#F3F1ED', border: '1px solid #C9C2B2', boxShadow: '0 30px 80px rgba(0,0,0,0.22),0 8px 20px rgba(0,0,0,0.10)', borderRadius: 6, overflow: 'hidden', fontFamily: F_UI, color: '#161310', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {/* Title bar */}
      <div style={{ height: 30, background: '#26303A', color: '#E4E1DA', display: 'flex', alignItems: 'center', padding: '0 14px', fontSize: 12, fontWeight: 500, gap: 10, opacity: ribbonOp }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
              {group.items.map(it => (
                <div key={it} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '2px 6px', borderRadius: 4 }}>
                  <div style={{ width: 28, height: 28, background: '#FFFFFF', border: '1px solid #DBD4C2', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2E3FFF', fontSize: 13, fontWeight: 700 }}>{it[0]}</div>
                  <div style={{ fontSize: 9.5, color: '#5C564B', letterSpacing: '0.01em' }}>{it}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize: 10, color: '#9A9384', textTransform: 'uppercase', letterSpacing: '0.08em', alignSelf: 'center' }}>{group.tab}</div>
          </div>
        ))}
      </div>
      {/* Body */}
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, background: '#D8D2C2', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          {children}
        </div>
        ...
        {[
          ...
          <div key={tr.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ flex: 1, height: 8, background: '#EBE5D4', borderRadius: 2, position: 'relative' }}>
              <div style={{ position: 'absolute', left: `${tr.x * 100}%`, width: `${tr.w * 100}%`, top: 0, bottom: 0, background: tr.color, borderRadius: 2 }} />
            </div>
          </div>
        ))}
      </div>
...
function StorylineCanvasContent({ build, titleEntry, charEntry, textEntry, btnEntry }: {
  ...
  return (
    <div style={{ width: 920, height: 520, background: CREAM, borderRadius: 4, position: 'relative', overflow: 'hidden', boxShadow: '0 12px 32px rgba(0,0,0,0.18)', border: '1px solid rgba(0,0,0,0.06)' }}>
      <div style={{ height: 50, background: 'rgba(255,255,255,0.7)', borderBottom: '1px solid rgba(0,0,0,0.06)', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', opacity: titleEntry }}>
        <AcmeLogo size={16} />
        <div style={{ width: 100, height: 4, background: primary, opacity: 0.9, borderRadius: 2 }} />
      </div>
      <div style={{ padding: '32px 40px', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 20, height: 'calc(100% - 50px)', alignItems: 'center' }}>
        ...
        <div style={{ opacity: charEntry, transform: `translateY(${(1 - charEntry) * 20}px) scale(${0.9 + 0.1 * charEntry})`, display: 'flex', justifyContent: 'center' }}>
          <CharacterAvatar size={190} />
        </div>
      </div>
      {build > 0.1 && build < 0.95 && (
        <div style={{ position: 'absolute', left: 30, top: 70, right: 30, bottom: 30, border: '1px dashed rgba(46,63,255,0.5)', pointerEvents: 'none' }}>
          {[[-4,-4,true,true],[-4,-4,true,false],[-4,-4,false,true],[-4,-4,false,false]].map((_,i) => (
            <div key={i} style={{ position: 'absolute', left: i < 2 ? -4 : undefined, right: i >= 2 ? -4 : undefined, top: i % 2 === 0 ? -4 : undefined, bottom: i % 2 !== 0 ? -4 : undefined, width: 8, height: 8, background: '#2E3FFF', borderRadius: 1 }} />
          ))}
        </div>
      )}
    </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// Character avatar — 4 styles, selected from context
// ─────────────────────────────────────────────────────────────────────────────

interface CharProps { size: number; primary: string; accent: string }

function GuideGeometric({ size, primary, accent }: CharProps) {
  return (
    <svg width={size} height={size * (300 / 260)} viewBox="0 0 260 300" style={{ filter: 'drop-shadow(0 22px 34px rgba(0,0,0,0.18))', overflow: 'visible' }}>
...
function GuideRiso({ size, primary, accent }: CharProps) {
  const pid = React.useId()
  return (
    <svg width={size} height={size * (320 / 260)} viewBox="0 0 260 320" style={{ filter: 'drop-shadow(0 22px 34px rgba(0,0,0,0.18))', overflow: 'visible' }}>
      <defs>
        <pattern id={`${pid}-dots`} x="0" y="0" width="6" height="6" patternUnits="userSpaceOnUse">
          <circle cx="3" cy="3" r="1.2" fill="#1A1714" opacity="0.35" />
        </pattern>
        <pattern id={`${pid}-dots-light`} x="0" y="0" width="6" height="6" patternUnits="userSpaceOnUse">
          <circle cx="3" cy="3" r="0.9" fill={primary} opacity="0.55" />
        </pattern>
      </defs>
      <rect x="6" y="6" width="248" height="308" rx="8" fill="#F1ECE0" />
      <rect x="6" y="6" width="248" height="308" rx="8" fill={`url(#${pid}-dots)`} opacity="0.18" />
      <g transform="translate(4,4)" opacity="0.85">
        <ellipse cx="130" cy="150" rx="58" ry="64" fill={accent} />
        <path d="M 78 250 C 84 200, 110 184, 130 184 C 150 184, 176 200, 182 250 L 182 300 L 78 300 Z" fill={accent} />
      </g>
      <g transform="translate(-3,-2)">
        <ellipse cx="130" cy="150" rx="56" ry="62" fill={primary} opacity="0.92" />
        <path d="M 80 250 C 86 200, 110 184, 130 184 C 150 184, 174 200, 180 250 L 180 300 L 80 300 Z" fill={primary} opacity="0.92" />
      </g>
      <ellipse cx="130" cy="150" rx="54" ry="60" fill={`url(#${pid}-dots-light)`} opacity="0.45" />
      <path d="M 80 132 C 78 96, 104 78, 130 78 C 156 78, 180 96, 182 132 C 168 116, 138 112, 130 130 C 116 116, 90 116, 80 132 Z" fill="#1A1714" />
      <circle cx="116" cy="146" r="3.2" fill="#1A1714" />
      <circle cx="144" cy="146" r="3.2" fill="#1A1714" />
      <path d="M 114 168 Q 130 180, 146 168" stroke="#1A1714" strokeWidth="3" fill="none" strokeLinecap="round" />
      <g stroke="#1A1714" strokeWidth="1" fill="none" opacity="0.5">
        <path d="M 14 14 L 14 26 M 14 14 L 26 14" />
        <path d="M 246 14 L 246 26 M 246 14 L 234 14" />
        <path d="M 14 306 L 14 294 M 14 306 L 26 306" />
        <path d="M 246 306 L 246 294 M 246 306 L 234 306" />
      </g>
    </svg>
  )
}

function GuideLine({ size, primary, accent }: CharProps) {
  return (
    <svg width={size} height={size * (300 / 260)} viewBox="0 0 260 300" style={{ filter: 'drop-shadow(0 22px 34px rgba(0,0,0,0.16))', overflow: 'visible' }}>
      <path d="M 36 80 C 60 30, 130 20, 188 50 C 230 70, 240 130, 220 180 C 200 230, 130 270, 80 240 C 30 210, 12 130, 36 80 Z" fill={primary} opacity="0.18" />
      <path d="M 90 100 C 88 70, 110 56, 134 58 C 168 60, 188 84, 184 124 C 184 140, 178 152, 170 162 C 164 174, 156 184, 150 196 L 150 218 C 178 220, 200 240, 208 274 L 208 300" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M 110 196 L 110 218 C 82 220, 60 240, 52 274 L 52 300" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M 90 130 C 84 134, 80 140, 80 148 C 80 154, 84 156, 90 156" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" />
      <path d="M 110 60 C 100 78, 96 100, 94 124" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" />
      <path d="M 108 124 Q 114 122, 120 124" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" />
      <path d="M 142 124 Q 148 122, 154 124" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" />
      <path d="M 132 140 L 130 156 L 138 158" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M 118 170 Q 132 180, 146 170" stroke={accent} strokeWidth="3" fill="none" strokeLinecap="round" />
      <circle cx="200" cy="80" r="8" fill={primary} />
    </svg>
  )
}

function GuideMascot({ size, primary, accent }: CharProps) {
  const sz = size
  return (
    <svg width={sz} height={sz * (300 / 260)} viewBox="0 0 260 300" style={{ filter: 'drop-shadow(0 22px 34px rgba(0,0,0,0.20))', overflow: 'visible' }}>
      {[{ x: 36, y: 70, s: 10 },{ x: 220, y: 90, s: 14 },{ x: 36, y: 230, s: 12 },{ x: 224, y: 250, s: 9 }].map((sp, i) => (
        <g key={i} transform={`translate(${sp.x},${sp.y})`} fill={primary} opacity="0.6">
          <path d={`M 0 -${sp.s} L ${sp.s*0.3} -${sp.s*0.3} L ${sp.s} 0 L ${sp.s*0.3} ${sp.s*0.3} L 0 ${sp.s} L -${sp.s*0.3} ${sp.s*0.3} L -${sp.s} 0 L -${sp.s*0.3} -${sp.s*0.3} Z`} />
        </g>
      ))}
      <path d="M 130 50 C 200 50, 230 110, 226 170 C 220 240, 180 270, 130 270 C 80 270, 40 240, 34 170 C 30 110, 60 50, 130 50 Z" fill={primary} />
      <path d="M 130 50 C 200 50, 230 110, 226 170 C 220 240, 180 270, 130 270 C 80 270, 40 240, 34 170 C 30 110, 60 50, 130 50 Z" fill="none" stroke="#fff" strokeWidth="6" opacity="0.4" />
      <ellipse cx="130" cy="190" rx="58" ry="62" fill="#fff" opacity="0.18" />
      <line x1="130" y1="50" x2="130" y2="22" stroke={accent} strokeWidth="3" strokeLinecap="round" />
      <circle cx="130" cy="18" r="6" fill={accent} />
      <circle cx="130" cy="18" r="2.5" fill="#fff" opacity="0.7" />
      <circle cx="100" cy="140" r="18" fill="#FFFFFF" />
      <circle cx="160" cy="140" r="18" fill="#FFFFFF" />
      <circle cx="103" cy="143" r="9"  fill="#1A1714" />
      <circle cx="163" cy="143" r="9"  fill="#1A1714" />
      <circle cx="106" cy="139" r="3.2" fill="#fff" />
      <circle cx="166" cy="139" r="3.2" fill="#fff" />
      <ellipse cx="82"  cy="178" rx="9" ry="6" fill={accent} opacity="0.45" />
      <ellipse cx="178" cy="178" rx="9" ry="6" fill={accent} opacity="0.45" />
      <path d="M 108 188 Q 130 218, 152 188 Q 130 200, 108 188 Z" fill="#1A1714" />
      <ellipse cx="130" cy="203" rx="6" ry="3" fill={accent} opacity="0.7" />
      <path d="M 36 180 Q 18 168, 22 150" stroke={primary} strokeWidth="14" fill="none" strokeLinecap="round" />
      <circle cx="22" cy="148" r="10" fill={primary} />
      <path d="M 224 180 Q 244 200, 240 220" stroke={primary} strokeWidth="14" fill="none" strokeLinecap="round" />
      <circle cx="240" cy="222" r="10" fill={primary} />
    </svg>
  )
}

function CharacterAvatar({ size }: { size: number }) {
  const { primary, accent, guide } = useTheme()
  const props: CharProps = { size, primary, accent }
  if (guide === 'riso')   return <GuideRiso    {...props} />
  if (guide === 'line')   return <GuideLine    {...props} />
  if (guide === 'mascot') return <GuideMascot  {...props} />
  return                         <GuideGeometric {...props} />
}
...
function Rosette({ x, y, size, color, opacity = 0.08 }: { x: number; y: number; size: number; color: string; opacity?: number }) {
  const teeth = 18, r1 = size / 2, r2 = r1 * 0.86
  const points: string[] = []
  ...
  return (
    <svg style={{ position: 'absolute', left: x, top: y, opacity, pointerEvents: 'none' }} width={size} height={size} viewBox={`${-r1} ${-r1} ${size} ${size}`}>
...
function Confetti({ x, y, color, angle = 0, size = 14 }: { x: number; y: number; color: string; angle?: number; size?: number }) {
  ...
  return (
    <svg width="320" height="320" viewBox="0 0 320 320" style={{ filter: `drop-shadow(0 24px 36px ${hexAlpha(primary, 0.22)})` }}>
      ...
      {Array.from({ length: 6 }).map((_, i) => {
        ...
        const sx = cx + Math.cos(a) * 90, sy = cy + Math.sin(a) * 90
...
function FeatureHeader({ label, tag }: { label: string; tag: string }) {
  const { primary, accent } = useTheme()
  return (
    <div style={{ padding: '18px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(0,0,0,0.06)', background: 'rgba(255,255,255,0.7)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <AcmeLogo size={16} />
        <span style={{ fontSize: 13, color: accent, fontWeight: 500, fontFamily: F_UI }}>{label}</span>
      </div>
      <div style={{ padding: '6px 12px', background: hexAlpha(primary, 0.10), color: primary, borderRadius: 999, fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: F_UI }}>{tag}</div>
    </div>
  )
}
  ...
  if (kind === 'cert') return (
    <div style={{ ...BASE, background: `radial-gradient(120% 80% at 20% 0%,${CREAM} 0%,#FFFFFF 60%,#FFFFFF 100%)` }}>
      ...
      <div style={{ position: 'absolute', left: 80, top: 140, width: 320, height: 320 }}>
        ...
        <Confetti x={-30} y={300}  color={accent}    angle={42}  size={10} />
      </div>
...
function BgSequential() {
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(120% 80% at 50% 40%,#F8F3E6 0%,#EDE6D4 100%)' }}>
  ...
  return (
    <Sprite start={0} end={8.4}>
      {({ localTime }) => {
        ...
        const absorbE = Easing.easeInOutCubic(clamp((localTime - 2.5) / 1.3, 0, 1))
        const zoomE   = Easing.easeInOutCubic(clamp((localTime - 3.8) / 2.6, 0, 1))
        const cracking = clamp((localTime - 7.0) / 1.0, 0, 1)
        const exitE   = Easing.easeInQuad(clamp((localTime - 8.0) / 0.4, 0, 1))
        const scale   = (0.92 + 0.08 * entryE + 0.03 * Math.sin(absorbE * Math.PI) + 0.30 * zoomE) * (1 - exitE * 0.05)
        const ty      = (1 - entryE) * 60
        const glow    = Math.max(0, Math.sin(absorbE * Math.PI) * 0.8)
        const CX = 960, CY = 540
        const convE = Easing.easeInCubic(clamp((localTime - 2.5) / 1.3, 0, 1))
        return (
          <>
            {localTime < 4.0 && THUMBS.map((th, i) => {
              const e   = Easing.easeOutBack(clamp((localTime - 0.4 - th.delay) / 1.2, 0, 1))
              const rad = th.angle * Math.PI / 180
              const x   = (CX + Math.cos(rad) * th.dist) + (CX - (CX + Math.cos(rad) * th.dist)) * convE
              const y   = (CY + Math.sin(rad) * th.dist) + (CY - (CY + Math.sin(rad) * th.dist)) * convE
              return (
                <div key={i} style={{ position: 'absolute', left: x, top: y, transform: `translate(-50%,-50%) rotate(${th.rot * e * (1 - convE)}deg) scale(${(0.5 + 0.5 * e) * (1 - 0.85 * convE)})`, opacity: e * (1 - Math.pow(convE, 3) * 0.95), transition: 'none' }}>
                  <PptThumbnail />
                </div>
              )
            })}
            <div style={{ position: 'absolute', left: CX, top: CY, transform: `translate(-50%,-50%) translateY(${ty}px) scale(${scale})`, opacity: entryE * (1 - exitE), filter: [glow > 0.01 ? `drop-shadow(0 0 ${glow * 30}px rgba(68,114,196,${glow * 0.45}))` : '', cracking > 0.5 ? `hue-rotate(${cracking * 4}deg) brightness(${1 - cracking * 0.06})` : ''].filter(Boolean).join(' ') || 'none' }}>
              <PptSlide />
              {cracking > 0.2 && <div style={{ position: 'absolute', left: -16, top: 200, width: 8, height: 320, background: '#FF6033', opacity: cracking, borderRadius: 4 }} />}
            </div>
          </>
        )
      }}
...
function BeatTransform() {
  const pieces = React.useMemo(() =>
    Array.from({ length: 36 }).map((_, i) => {
      const rng = (s: number) => { const v = Math.sin(i * 12.9898 + s * 78.233) * 43758.5453; return v - Math.floor(v) }
      return { startX: 880 + rng(1) * 160, startY: 380 + rng(2) * 320, endX: 1380 + rng(3) * 200, endY: 460 + rng(4) * 200, rot0: (rng(5) - 0.5) * 40, rot1: (rng(5) - 0.5) * 720, w: 30 + rng(6) * 80, h: 6 + rng(7) * 6, color: ['#4472C4','#1F4E79','#262626','#FF6033'][Math.floor(rng(8) * 4)], delay: rng(9) * 0.6 }
    })
  , [])
  return (
      ...
      {({ localTime }) => (
        <>
          {pieces.map((p, i) => {
            ...
            return <Debris key={i} x={p.startX + (p.endX - p.startX) * e} y={p.startY + (p.endY - p.startY) * e - Math.sin(e * Math.PI) * 120} w={p.w} h={p.h} rot={p.rot0 + (p.rot1 - p.rot0) * e} color={p.color} opacity={clamp(t * 6, 0, 1) * clamp(1 - (localTime - 1.8) / 1.0, 0, 1)} />
          })}
          <div style={{ position: 'absolute', left: 0, top: '50%', right: 0, height: 1, background: 'linear-gradient(90deg,transparent 30%,#FF6033 50%,transparent 70%)', opacity: clamp(Math.sin(localTime * 2.5) * 0.6, 0, 0.6), transform: 'translateY(-50%)' }} />
        </>
      )}
    </Sprite>
  )
...
function BeatStorylineAssemble() {
  return (
    <Sprite start={8.5} end={16.05}>
      {({ localTime }) => {
        const entryE  = Easing.easeOutCubic(clamp(localTime / 0.7, 0, 1))
        const buildE  = Easing.easeOutCubic(clamp((localTime - 0.4) / 2.6, 0, 1))
        const drift   = Math.sin(localTime * 0.4) * 6
        const exitE   = Easing.easeInQuad(clamp((localTime - 6.8) / 0.7, 0, 1))
        return (
          <div style={{ position: 'absolute', left: '50%', top: '50%', transform: `translate(-50%,-50%) translate(${drift}px,${-drift * 0.3}px) scale(${(0.86 + 0.12 * entryE) * (1 - exitE * 0.05)})`, opacity: entryE * (1 - exitE) }}>
            <StorylineEditor build={buildE}>
              <StorylineCanvasContent
                build={clamp((localTime - 2.8) / 3.0, 0, 1)}
                titleEntry={Easing.easeOutBack(clamp((localTime - 3.0) / 0.6, 0, 1))}
                charEntry={Easing.easeOutCubic(clamp((localTime - 3.8) / 0.7, 0, 1))}
                textEntry={Easing.easeOutCubic(clamp((localTime - 4.6) / 0.5, 0, 1))}
                btnEntry={Easing.easeOutBack(clamp((localTime - 5.4) / 0.5, 0, 1))}
              />
            </StorylineEditor>
          </div>
        )
...
function BeatPublishedReveal() {
  return (
    <Sprite start={15.5} end={20.6}>
      {({ localTime }) => {
        const flash  = Math.min(clamp(localTime / 0.4, 0, 1), Math.max(0, 1 - clamp((localTime - 0.8) / 0.4, 0, 1)))
        const entryE = Easing.easeOutCubic(clamp((localTime - 0.6) / 0.8, 0, 1))
        const exitE  = Easing.easeInCubic(clamp((localTime - 4.5) / 0.5, 0, 1))
        const kb     = 1 + 0.04 * Easing.easeOutCubic(clamp(localTime / 5.0, 0, 1))
        return (
          <>
            <div style={{ position: 'absolute', left: '50%', top: '50%', transform: `translate(-50%,-50%) scale(${(0.95 + 0.05 * entryE) * kb * (1 - exitE * 0.04)})`, opacity: entryE * (1 - exitE) }}>
              <PublishedSlide
                titleEntry={Easing.easeOutCubic(clamp((localTime - 1.2) / 0.6, 0, 1))}
                characterEntry={Easing.easeOutBack(clamp((localTime - 1.6) / 0.6, 0, 1))}
                ctaEntry={Easing.easeOutBack(clamp((localTime - 2.2) / 0.5, 0, 1))}
                progress={0.12 + 0.06 * entryE}
              />
            </div>
            <div style={{ position: 'absolute', inset: 0, background: '#fff', opacity: Easing.easeInOutCubic(flash), pointerEvents: 'none', zIndex: 40 }} />
          </>
        )
      }}
    </Sprite>
  ...
  return (
    <Sprite start={20.4} end={25.0}>
      {({ localTime }) => (
        <>
          {SLOTS.map((s, i) => {
            const t = localTime - s.t0, dur = s.t1 - s.t0
            ...
            return (
              <div key={i} style={{ position: 'absolute', left: '50%', top: '50%', transform: `translate(-50%,-50%) translateY(${(1 - inE) * 30 - outE * 20}px) scale(${0.95 + 0.05 * inE})`, opacity: Math.min(inE, 1 - outE) }}>
...
export interface SceneSistelProps {
  primary?: string
  accent?:  string
  guide?:   GuideStyle
}
