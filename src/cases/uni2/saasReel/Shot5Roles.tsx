...
const MODULE_CONTENT: Record<string, { form: ContentRow[]; visual: ContentRow[] }> = {
  'pre-approver': {
    form: [{ label: 'Documento', value: '1.024.578.331' }, { label: 'Score básico', value: 'consultando…' }, { label: 'Listas', value: 'verifik · ofac' }],
    visual: [{ label: 'Pre-score', value: '720 · ok' }, { label: 'Listas', value: '✓ limpio' }, { label: 'Verifik', value: '✓' }],
  },
  'info-client': {
    form: [{ label: 'Cédula', value: '1.024.578.331' }, { label: 'Nombre', value: 'Tatiana Avilés' }, { label: 'Nacida', value: '14·06·1991' }],
    visual: [{ label: 'Cédula', value: '1.024.578.331' }, { label: 'Nombre', value: 'Tatiana A.' }, { label: 'Edad', value: '34 años' }],
  },
  'info-credit': {
    form: [{ label: 'Monto', value: '$ 28.000.000' }, { label: 'Plazo', value: '36 meses' }, { label: 'Marca', value: 'Toyota' }],
    visual: [{ label: 'Monto', value: '$ 28.0M' }, { label: 'Plazo', value: '36 m' }, { label: 'Cuota', value: '$ 1.04M' }],
  },
  'info-contact': {
    form: [{ label: 'Celular', value: '+57 312 478··' }, { label: 'Email', value: 'tati@indu…' }, { label: 'Ciudad', value: 'Medellín' }],
    visual: [{ label: 'Celular', value: '312 478··' }, { label: 'Email', value: 'tati@…' }, { label: 'Ciudad', value: 'Medellín' }],
  },
  insurability: {
    form: [{ label: 'PPI', value: 'declara ▾' }, { label: 'Cuota', value: '$ 38.400 / mes' }, { label: 'Salud', value: 'declara ▾' }],
    visual: [{ label: 'PPI', value: '✓ acepta' }, { label: 'Salud', value: 'sin reservas' }, { label: 'Cuota', value: '$ 38.4k' }],
  },
  'data-authorization': {
    form: [{ label: 'Habeas', value: 'leer ▾' }, { label: 'Firma', value: 'pendiente' }, { label: 'Token', value: '—' }],
    visual: [{ label: 'Habeas', value: '✓ aceptado' }, { label: 'Firma', value: '✓ firmada' }, { label: 'Token', value: 'fxk-93' }],
  },
  documents: {
    form: [{ label: 'CC frente', value: 'subir ↑' }, { label: 'CC rev.', value: 'subir ↑' }, { label: 'Desprend.', value: 'subir ↑' }],
    visual: [{ label: 'Cédula', value: '✓ verificado' }, { label: 'Desprend.', value: '✓ 2 meses' }, { label: 'RUT', value: '✓ vigente' }],
  },
  decision: {
    form: [{ label: 'Resultado', value: 'Aprobado ▾' }, { label: 'Monto fin.', value: '$ 28.0M' }, { label: 'Comentario', value: '—' }],
    visual: [{ label: 'Score', value: '742 · A−' }, { label: 'Reglas', value: '6/6' }, { label: 'Sugerencia', value: 'aprobar' }],
  },
  default: {
    form: [{ label: 'Campo 1', value: '—' }, { label: 'Campo 2', value: '—' }, { label: 'Campo 3', value: '—' }],
    visual: [{ label: 'Dato 1', value: '—' }, { label: 'Dato 2', value: '—' }, { label: 'Dato 3', value: '—' }],
  },
}

type CardState = 'pending' | 'active' | 'done'

const EXPEDIENTE = { id: 'SOL-04781', name: 'Tatiana Avilés Restrepo', amount: '$ 28.0M', product: 'vehicle credit' }
const LANE_TOP = 318
const LANE_GAP = 22
...
ROLE_CONFIGS.forEach((r, ri) => r.steps.forEach((s, si) => flatSteps.push({ roleIdx: ri, stepIdx: si, step: s })))
const TOTAL_STEPS = flatSteps.length
const COMPLETED_AT = TRAVEL_START + TOTAL_STEPS * STEP_DUR
...
export const Shot5Roles = React.memo(function Shot5Roles() {
  const cameraRef = React.useRef<HTMLDivElement>(null)
  const titleRef = React.useRef<HTMLDivElement>(null)

  const [cursorIdx, setCursorIdx] = React.useState(-1)
  const prevCursorRef = React.useRef(-1)

  useSpriteEffect((lt) => {
    const camScale = 0.97 + Easing.easeOutCubic(clamp(lt / 1.5, 0, 1)) * 0.03
    if (cameraRef.current) cameraRef.current.style.transform = `scale(${camScale})`

    const titleEntry = clamp((lt - 0.2) / 0.6, 0, 1)
    if (titleRef.current) {
      titleRef.current.style.opacity = String(titleEntry)
      titleRef.current.style.transform = `translateY(${(1 - titleEntry) * 10}px)`
    }

    const newCursorIdx = Math.floor((lt - TRAVEL_START) / STEP_DUR)
    if (newCursorIdx !== prevCursorRef.current) {
      prevCursorRef.current = newCursorIdx
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontFamily: 'var(--f-mono)', fontSize: 13, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#818CF8', marginBottom: 8 }}>
          07 · una solicitud · tres roles · una ruta
        </div>
        <div style={{ fontFamily: 'var(--f-display)', fontSize: 50, color: '#ECECEA', lineHeight: 1.0, maxWidth: 1700, letterSpacing: '-0.02em' }}>
          Same file. <span style={{ fontStyle: 'italic' }}>Every role.</span>
        </div>
        <div style={{ fontFamily: 'var(--f-mono)', fontSize: 13, color: 'rgba(236,236,234,0.55)', marginTop: 12, letterSpacing: '0.04em' }}>
      ...
      <ExpedienteIdentityChip ticker={tickerStr} completed={completed} />

      {completed && <AprobadoSeal />}
...
function JourneyLane({
  ...
  useSpriteEffect((lt) => {
    const lanePresent = clamp((lt - startDelay) / 0.5, 0, 1)
    if (outerRef.current) outerRef.current.style.opacity = String(lanePresent)
  })
  ...
  return (
    <div ref={outerRef} style={{ position: 'relative', opacity: 0 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 10 }}>
        <div style={{ fontFamily: 'var(--f-mono)', fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: role.accent, opacity: 0.9, fontWeight: 600 }}>
          Rol
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
        {role.steps.map((step, i) => {
          ...
          const entryStartAt = startDelay + i * cardStagger
          return (
...
const RoleModuleCard = React.memo(function RoleModuleCard({
  step,
  entryStartAt,
  cardDur,
  accent = '#818CF8',
  ...
  useSpriteEffect((lt) => {
    const entryProgress = clamp((lt - entryStartAt) / cardDur, 0, 1)
    const ty = (1 - entryProgress) * -14
    const scale = 0.92 + 0.08 * Easing.easeOutBack(entryProgress)
    if (outerRef.current) {
      outerRef.current.style.opacity = String(entryProgress)
      outerRef.current.style.transform = `translateY(${ty}px) scale(${scale})`
    }
  })
  ...
  return (
    <div ref={outerRef} style={{ opacity: 0, transform: 'translateY(-14px) scale(0.92)', flexShrink: 0, willChange: 'transform, opacity' }}>
      <div
        style={{
          width: 168,
          height: 116,
...
function ChevronConnect({
  ...
  useSpriteEffect((lt) => {
    const entry = clamp((lt - entryStartAt) / cardDur, 0, 1)
    if (divRef.current) divRef.current.style.opacity = String(clamp((entry - 0.3) * 2, 0, 1))
  })

  return (
    <div ref={divRef} style={{ width: 22, height: 116, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, flexShrink: 0 }}>
...
function LaneStatusBadge({ status, accent, ticker }: { status: 'waiting' | 'active' | 'done'; accent: string; ticker: string | null }) {
  ...
  return (
    <div
      style={{
        ...
        border: isActive ? `1px solid ${accent}80` : isDone ? '1px solid rgba(95,212,154,0.4)' : '1px solid rgba(255,255,255,0.1)',
        borderRadius: 999,
        fontFamily: 'var(--f-mono)',
        fontSize: 10,
...
function ExpedienteIdentityChip({ ticker, completed }: { ticker: string; completed: boolean }) {
  const outerRef = React.useRef<HTMLDivElement>(null)
  const tickerRef = React.useRef<HTMLDivElement>(null)

  useSpriteEffect((lt) => {
    const enter = clamp((lt - 1.6) / 0.6, 0, 1)
    if (outerRef.current) {
      outerRef.current.style.opacity = String(enter)
      outerRef.current.style.transform = `translateX(${(1 - enter) * 24}px)`
    }
    if (enter <= 0) return
    const cursorIdx = Math.floor((lt - TRAVEL_START) / STEP_DUR)
    const tickMinute = 41 + (cursorIdx < 0 ? 0 : cursorIdx)
    const h = 10 + Math.floor(tickMinute / 60)
    const m = tickMinute % 60
    if (tickerRef.current) tickerRef.current.textContent = `${h}:${String(m).padStart(2, '0')}`
  })
  ...
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(236,236,234,0.45)' }}>
...
function AprobadoSeal() {
  const outerRef = React.useRef<HTMLDivElement>(null)

  useSpriteEffect((lt) => {
    const t = clamp((lt - COMPLETED_AT) / 0.45, 0, 1)
    const scale = 0.6 + Easing.easeOutBack(t) * 0.4
    if (outerRef.current) {
      outerRef.current.style.opacity = String(t)
  ...
  return (
    ...
    >
      <div style={{ width: 32, height: 32, borderRadius: 16, background: '#5fd49a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg width="16" height="16" viewBox="0 0 18 18">
          <path d="M3 9l4 4 8-8" stroke="#06291d" strokeWidth="2.2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
      <div>
        <div style={{ fontFamily: 'var(--f-mono)', fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(95,212,154,0.75)' }}>
          decisión
        </div>
        <div style={{ fontFamily: 'var(--f-display)', fontSize: 28, color: '#ECECEA', lineHeight: 1, marginTop: 2, fontStyle: 'italic' }}>
          Aprobado
        </div>
      </div>
    </div>
...
function ModeBadge({ mode, accent }: { mode: 'form' | 'visual'; accent: string }) {
  ...
  return (
    ...
    >
      <span style={{ width: 5, height: 5, borderRadius: 3, background: isForm ? accent : 'rgba(236,236,234,0.55)', boxShadow: isForm ? `0 0 5px ${accent}` : 'none' }} />
...
function FormBody({ fields }: { fields: ContentRow[] }) {
  return (
    <>
      {fields.map((f, i) => (
        <div key={i}>
          <div style={{ fontFamily: 'var(--f-mono)', fontSize: 7.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(122,130,148,0.85)', marginBottom: 2 }}>
            {f.label}
          </div>
          <div style={{ height: 16, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 3, padding: '0 5px', display: 'flex', alignItems: 'center', fontSize: 9, color: 'rgba(236,236,234,0.88)', fontVariantNumeric: 'tabular-nums' }}>
            {f.value}
          </div>
        </div>
      ))}
...
function VisualBody({ rows }: { rows: ContentRow[] }) {
  return (
    <>
      {rows.map((r, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 6, justifyContent: 'space-between' }}>
          <span style={{ fontFamily: 'var(--f-mono)', fontSize: 7.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(122,130,148,0.85)', flexShrink: 0 }}>
            {r.label}
          </span>
          <span style={{ fontSize: 9.5, color: '#ECECEA', fontWeight: 500, fontVariantNumeric: 'tabular-nums', textAlign: 'right', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {r.value}
          </span>
        </div>
      ))}
